...
function getHandler(options, proxy) {
  var corsAnywhere = {
    ...
    proxyBasePath: '',              // Optional path to prefix before the proxied URL.
  };
  ...
  if (corsAnywhere.proxyBasePath) {
    corsAnywhere.proxyBasePath = String(corsAnywhere.proxyBasePath).replace(/^\/+|\/+$/g, '');
    corsAnywhere.proxyBasePath = corsAnywhere.proxyBasePath ? '/' + corsAnywhere.proxyBasePath : '';
  }
  ...
  return function(req, res) {
    ...
    var proxyBasePath = corsAnywhere.proxyBasePath;
    var requestPath = req.url;
    if (proxyBasePath) {
      if (requestPath.lastIndexOf(proxyBasePath + '/', 0) === 0) {
        requestPath = requestPath.slice(proxyBasePath.length);
      } else {
    ...
    var location = parseURL(requestPath.slice(1));
    ...
    if (!/^\/https?:/.test(requestPath) && !isValidHostName(location.hostname)) {
    ...
    var proxyBaseUrl = (isRequestedOverHttps ? 'https://' : 'http://') + req.headers.host + proxyBasePath;
